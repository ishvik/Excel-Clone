...
//loop for making 100 cells with numeric row no. and alphabetic column no.
for (let i = 1; i <= 100; i++) {
    let str = "";
    let n = i;
    ...
    $("#columns").append(`<div class="column-name column-${i}" id="${str}">${str}</div>`)   //adding columns
    $("#rows").append(`<div class="row-name">${i}</div>`)                                  //adding rows 
}

//store all data of sheets
let cellData = {
    "Sheet1": {}
};

//checking whether file is saved or not
let save = true;
...
//all by-defualt properties of a cell
...
//giving unique id and "input-cell" class to all cells 
for (let i = 1; i <= 100; i++) {
...
//making columns scrollable to left
//making rows scrollable to top
$("#cells").scroll(function (e) {
    $("#columns").scrollLeft(this.scrollLeft);
    $("#rows").scrollTop(this.scrollTop);
});

//making any cell editable by double clicking on it
$(".input-cell").dblclick(function (e) {
    $(".input-cell.selected").removeClass("selected top-selected bottom-selected left-selected right-selected"); //remove selected class from other cells
    $(this).addClass("selected");
    $(this).attr("contenteditable", "true");
    $(this).focus();
});

//cell become uneditable when clicked on somewhere else
$(".input-cell").blur(function (e) {
    $(this).attr("contenteditable", "false");
    updateCellData("text", $(this).text());
});

//getting id of row and column
...
//getting id of right cell, left cell, top cell, bottom cell of a particular cell
...
//selecting and unselecting multiple cells through ctrl key
$(".input-cell").click(function (e) {
    let [rowId, colId] = getRowCol(this);
    let [topCell, bottomCell, leftCell, rightCell] = getTopLeftBottomRightCell(rowId, colId);
    if ($(this).hasClass("selected") && e.ctrlKey) {                    //if a cell is selected then unselect
        unselectCell(this, e, topCell, bottomCell, leftCell, rightCell);
    } else {                                                            //else select that cell
        selectCell(this, e, topCell, bottomCell, leftCell, rightCell);
    }
});


//reflecting every changes to header as done in cell
function changeHeader([rowId, colId]) {
    ...
    $(".alignment.selected").removeClass("selected");                //reflecting alignment
    $(`.alignment[data-type=${data.alignment}]`).addClass("selected");
    addRemoveSelectFromFontStyle(data, "bold");                     //reflecting font-bold
    addRemoveSelectFromFontStyle(data, "italic");                   //reflecting font-italic
    addRemoveSelectFromFontStyle(data, "underlined");               //reflecting style-underlined
    $("#fill-color").css("border-bottom", `4px solid ${data.bgcolor}`); //reflecting background-color
    $("#text-color").css("border-bottom", `4px solid ${data.color}`);   //reflecting text-color
    $("#font-family").val(data["font-family"]);                         //reflecting font-family
    $("#font-size").val(data["font-size"]);                             //reflecting font-size
    $("#font-family").css("font-family", data["font-family"]);
}

//selecting and unselecting font style
...
//selecting all cell between given starting row-col and ending row-col
...
//variable used for selecting cell through mouse move
let count = 0;
...
//selecting multiple cell through mouse
$(".input-cell").mousemove(function (e) {
    e.preventDefault();                //removing default function of left click of mouse
    if (e.buttons == 1) {
        if (e.pageX > ($(window).width() - 10) && !scrollXRStarted) {  //checking if left scrollbale is false && X-axis value of target is greater than window's width 
            scrollXR();                                               //if true start scrolling to right
        } else if (e.pageX < (10) && !scrollXLStarted) {            //else start scrolling left
            scrollXL();                                                
        }
        //selecting cell
        if (!startcellSelected) {
            let [rowId, colId] = getRowCol(this);            //getting row and col id of target cell
            startCell = { "rowId": rowId, "colId": colId }; //storing ut in startcell
            selectAllBetweenCells(startCell, startCell);    //selecting all cells
...
//update celldata whenever there is any changes in any cell
function updateCellData(property, value) {
    let currCellData = JSON.stringify(cellData);
    if (value != defaultProperties[property]) {  //if there is any change
...
//change bold style of cell
$("#bold").click(function (e) {
    setStyle(this, "bold", "font-weight", "bold");
});

//change italic style of cell
$("#italic").click(function (e) {
    setStyle(this, "italic", "font-style", "italic");
});

//change underlined style of cell
$("#underlined").click(function (e) {
    setStyle(this, "underlined", "text-decoration", "underline");
});

//function to set any style given as value to a key in params
...
//color picker for bg-color and text-color
...
function openFile(){
    ...
    inputFile.change(function(e){
        ...
        reader.onload = () =>{
            ...
            for(let i of sheets){
                if(i.includes("Sheet")){
                    let splitSheet = i.split("Sheet");
                    if(splitSheet.length == 2 && !isNaN(splitSheet[1])){
                        lastlyAddedSheet = parseInt(splitSheet[1]);
                    }
                }
